...
double Co_vol(double theta1, double theta2, double phi1, double phi2, double z1, double z2) {
    ...
    double theta1_r = theta1 * (M_PI / 180);
    double theta2_r = theta2 * (M_PI / 180);
    double theta_v = abs(cos(theta1_r) - cos(theta2_r));
    double phi_v;
    if(phi1 < phi2){
        phi_v = abs(phi1 - phi2);
    }
    else if (phi1 > phi2) {
        phi_v = abs((2*M_PI - phi1) + phi2);
    }
    ...
    F.params = &params;
    //calculating r1
    gsl_integration_qags(&F, low, z1,
        abs_error, rel_error, 1000, work_ptr, &result,
        &error);
    double r1 = result;
    //calculating r2
    gsl_integration_qags(&F, low, z2,
        abs_error, rel_error, 1000, work_ptr, &result,
        &error);
    double r2 = result;
...
int main(void) {
    ...
    double phi1 = (RA1h + RA1m / 60.0 + RA1s / 3600) * 15 * 0.0174533;
    double phi2 = (RA2h + RA2m / 60.0 + RA2s / 3600) * 15 * 0.0174533;
    //cout << theta1 << endl;
    //cout << theta2 << endl;
